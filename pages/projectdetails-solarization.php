...
$average_output = explode('|', $project->project_details->average_output);
$output = array();
foreach($average_output as $item){
	array_push($output, (float)$item);
}
			...
			<div class="col-12 parameters">
				...
				<table class="table table-sm">
					...
					<tbody>
						...
						<tr>
							<td <?php echo $pv_disconnect->pv_disconnect->n->other->quantity > 0 ? 'rowspan="2"' : ''; ?>>PV Disconnect</td>
							<td style="text-align: center;"><?php echo $pv_disconnect->pv_disconnect->n->quantity ?></td>
							<td><?php echo $pv_disconnect->pv_disconnect->pv_disconnect_model ?></td>
						</tr>
						<?php if($pv_disconnect->pv_disconnect->n->other->quantity > 0){ ?>
						<tr>
							<td style="text-align: center;"><?php echo $pv_disconnect->pv_disconnect->n->other->quantity ?></td>
							<td><?php echo $pv_disconnect->pv_disconnect->n->other->model ?></td>
						</tr>
						<?php } ?>
						<tr>
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col footer-buttons">
				...
				<?php } else { ?>
				<button class="btn btn-primary print-solarization-pdf float-right" data-project="<?php echo $get->project_id; ?>">Print Project <i class="far fa-save"></i></button>
				<?php } ?>
			</div>
		</div>
	</div>
		...
		if(product_id != 0){
			$.ajax({
		        ...
		        success: function(data, status, xhr){
		        	...
		        	// Product Title
		        	containercontent += '<h3>' +data.product_meta_description+ '</h3>';
		        	// Product Images
